...
class TestGetAPIErrors:
    def test_invalid_json_raises_api_error(self):
        ...
        with (
            patch("wicli.core.client.urlopen", return_value=mock),
            pytest.raises(WicliAPIError, match="Invalid JSON"),
        ):
    ...
    def test_http_404_raises_api_error(self):
        ...
        with (
            patch("wicli.core.client.urlopen", side_effect=err),
            pytest.raises(WicliAPIError, match="HTTP error: 404"),
        ):
            _get("http://example.com")
    ...
    def test_http_500_raises_api_error(self):
        ...
        with (
            patch("wicli.core.client.urlopen", side_effect=err),
            pytest.raises(WicliAPIError, match="HTTP error: 500"),
        ):
            _get("http://example.com")
    ...
    def test_http_429_exhausted_raises_api_error(self):
        ...
        with (
            patch("wicli.core.client.urlopen", side_effect=err),
            patch("wicli.core.client.time.sleep"),
            pytest.raises(WicliAPIError, match="Too many requests"),
        ):
            _get("http://example.com")
...
class TestGetNetworkErrors:
    ...
    def test_timeout_raises_network_error(self):
        with (
            patch("wicli.core.client.urlopen", side_effect=TimeoutError()),
            pytest.raises(WicliNetworkError, match="timed out"),
        ):
            _get("http://example.com")
...
class TestRetryLogic:
    def test_429_retries_max_retries_times(self):
        ...
        with (
            patch("wicli.core.client.urlopen", side_effect=err),
            patch("wicli.core.client.time.sleep") as mock_sleep,
            pytest.raises(WicliAPIError),
        ):
            _get("http://example.com")
    ...
    def test_succeeds_after_retry(self):
        ...
        with (
            patch("wicli.core.client.urlopen", side_effect=[err, success]),
            patch("wicli.core.client.time.sleep"),
        ):
            result = _get("http://example.com")
        assert result == {"ok": True}
...
class TestCachedDecorator:
    ...
    def test_cache_miss_calls_fn_and_saves(self):
        ...
        with (
            patch("wicli.core.client.cache.load", return_value=None) as mock_load,
            patch("wicli.core.client.cache.save") as mock_save,
            patch("wicli.core.client.urlopen", return_value=_mock_response(api_data)),
        ):
    ...
    def test_cache_hit_returns_cached_without_http(self):
        ...
        with (
            patch("wicli.core.client.cache.load", return_value=cached_data),
            patch("wicli.core.client.urlopen") as mock_urlopen,
        ):
            result = client_module.fetch_summary("Python", lang="en")
    ...
    def test_disable_cache_bypasses_cache(self):
        ...
        with (
            patch("wicli.core.client.cache.load") as mock_load,
            patch("wicli.core.client.cache.save") as mock_save,
            patch("wicli.core.client.urlopen", return_value=_mock_response(api_data)),
        ):
            result = client_module.fetch_summary("Python", lang="en")
